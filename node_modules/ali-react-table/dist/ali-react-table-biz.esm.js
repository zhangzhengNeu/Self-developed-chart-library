...
import { h as _objectWithoutProperties, I as Icon, j as _extends, k as STYLED_VERSION, b as isLeafNode, c as collectNodes, m as makeTreeModeTransform, t as treeMetaSymbol } from './chunks/ali-react-table-treeMode-d713dac0.esm.js';
...
import { v as makeVisibleTransform, c as makeAutoRowSpanTransform, d as makeBuildTreeTransform, f as makeColumnHoverTransform, g as makeColumnRangeHoverTransform, i as makeColumnResizeTransform, k as makeFlattenTransform, n as makeOrderFieldTransform, o as makeSortTransform, r as makeTipsTransform, t as traverseColumn } from './chunks/ali-react-table-visible-9be3648a.esm.js';
...
function CustomColumnsDialog({ trigger, defaultVisible, defaultCheckedCodes, enforceCheckedCodes, checkedCodes, onChangeCheckedCodes, columns, components, style, className, showCheckAll, showUncheckAll, drawerProps, drawerComponent: drawerComponentProp, }) {
    ...
    function dfs(columns) {
        ...
        for (let colIndex = 0; colIndex < columns.length; colIndex++) {
            ...
            if (isLeafNode(col)) {
                if (col.code) {
                    ...
                    addToTempLine(React.createElement(Checkbox, { key: colIndex, className: "checkbox", checked: checked || enforce, disabled: enforce, onChange: () => onCheck([col.code], checked ? 'uncheck' : 'check') }, col.name));
                ...
                flushTempLine(true);
                nodes.push(React.createElement("div", { className: "group", key: colIndex },
                    React.createElement("div", { className: "group-header" },
                        React.createElement(Checkbox, { className: "checkbox", checked: sub.allCheck, indeterminate: !sub.allCheck && sub.anyCheck, onChange: () => onCheck(sub.codes, sub.allCheck ? 'uncheck' : 'check') }, col.name)),
                    React.createElement("div", { className: "group-content" }, sub.nodes)));
            }
        }
        flushTempLine(true);
        ...
        function flushTempLine(force) {
            ...
            if ((force && tempLine.length > 0) || tempLine.length >= FLAT_COUNT) {
                nodes.push(React.createElement("div", { className: "line", key: `line-${lineCount}` }, tempLine));
                lineCount += 1;
                tempLine = [];
            }
        }
    ...
    const content = (React.createElement(React.Fragment, null,
        React.createElement(Header, null,
            React.createElement("span", { style: { fontSize: 14 } }, "\u81EA\u5B9A\u4E49\u663E\u793A\u5217"),
            React.createElement("div", { className: "actions" },
                defaultCheckedCodes && (React.createElement("span", { className: "action", onClick: () => updateEditingCheckedCodes(defaultCheckedCodes) }, "\u6062\u590D\u9ED8\u8BA4")),
                showCheckAll && (React.createElement("span", { className: "action", onClick: () => onCheck(dfsResult.codes, 'check') }, "\u5168\u9009")),
                showUncheckAll && (React.createElement("span", { className: "action", onClick: () => onCheck(dfsResult.codes, 'uncheck') }, "\u5168\u4E0D\u9009")))),
        React.createElement("main", null,
            React.createElement(TreeDiv, null, dfsResult.nodes)),
        React.createElement(Footer, null,
            React.createElement(Button, { type: "primary", onClick: onOk }, "\u786E\u8BA4"),
            React.createElement(Button, { style: { marginLeft: 8 }, onClick: onClose }, "\u53D6\u6D88"))));
    return (React.createElement(React.Fragment, null,
        trigger(onOpen),
        drawerComponentProp
            ? React.createElement(drawerComponentProp, props, content)
            : React.createElement(StyledDiv, Object.assign({ as: DrawerComponent }, props), content)));
}
...
function useOperationBar({ features, columns, components, actionColor = '#23a3ff', }) {
    ...
    for (let index = 0; index < features.length; index++) {
        ...
        if (feat.type === 'separator') {
            nodes.push(React.createElement("span", { className: "item separator", key: index }));
        }
            ...
            nodes.push(React.createElement(CustomColumnsDialog, { key: index, columns: columns, drawerComponent: feat.drawerComponent, checkedCodes: visibleCodes, onChangeCheckedCodes: onChangeVisibleCodes, defaultCheckedCodes: codeGroups.defaultVisible, enforceCheckedCodes: codeGroups.enforceVisible, trigger: (onOpen) => (React.createElement("span", { className: "item action", onClick: onOpen },
                    React.createElement(Column16, { className: "icon" }),
            ...
            nodes.push(React.createElement("span", { key: index, className: "item action", onClick: feat.onExport },
                React.createElement(Download16, { className: "icon" }),
                "\u5BFC\u51FA"));
        }
        else if (feat.type === 'click') {
            nodes.push(React.createElement("span", { key: index, className: "item action", onClick: feat.onClick }, feat.title));
        }
        else if (feat.type === 'toggle') {
            const checked = (_a = map[feat.key]) !== null && _a !== void 0 ? _a : false;
            nodes.push(React.createElement(Checkbox, { key: index, className: "item", checked: checked, onChange: () => setMap(Object.assign(Object.assign({}, map), { [feat.key]: !map[feat.key] })) }, feat.title));
            if (feat.transform) {
            ...
            if (feat.node) {
                nodes.push(React.createElement("span", { className: "item", key: index }, feat.node));
            }
            if (feat.transform) {
    ...
    const view = useCallback((props) => (React.createElement(OperationBarDiv, Object.assign({}, props, { style: Object.assign({ '--action-color': actionColor }, props.style) }))), [OperationBarDiv, actionColor]);
...
const transformColumn = deprecate(traverseColumn, 'transformColumn', 'traverseColumn');
...
export { CustomColumnsDialog, commonTransforms, transformColumn, useOperationBar };
