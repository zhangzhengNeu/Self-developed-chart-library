...
function buildDrillTree(data, codes, { encode = simpleEncode, totalValue = '总计', includeTopWrapper = false, isExpand = () => true, enforceExpandTotalNode = true, } = {}) {
    ...
    function dfs(slice, path) {
        ...
        const groups = treeMode.groupBy(slice, (row) => row[code]);
...
function buildRecordMatrix({ data, leftCodes, topCodes, aggregate = fallbackAggregate, encode = simpleEncode, isLeftExpand = () => true, isTopExpand = () => true, prebuiltLeftTree, prebuiltTopTree, }) {
    ...
    function buildByLeft(ctx, slice, drillNode, depth) {
        let children = null;
        let col;
        if (treeMode.isLeafNode(drillNode)) {
            col = buildByTop(ctx, slice, topRootDrillNode, 0);
        }
        else {
            children = [];
            const code = leftCodes[depth];
            const groups = treeMode.groupBy(slice, (dwdRow) => dwdRow[code]);
    ...
    function mergeColsByTopTree(ctx, colsToMerge) {
        ...
        function dfs(ctx, cols, topDrillNode, depth) {
            ...
            if (!treeMode.isLeafNode(topDrillNode)) {
                const topCode = topCodes[depth];
                ctx.peculiarity.add(topCode);
                const drillChildDict = treeMode.fromEntries(topDrillNode.children.map((child) => [child.value, child]));
                const colChildDictArray = cols.map((col) => treeMode.fromEntries(col.children.map((child) => [child.topValue, child])));
...
function convertDrillTreeToCrossTree(drillTree, { indicators, encode = simpleEncode, generateSubtotalNode, enforceExpandTotalNode = true, expandKeys, onChangeExpandKeys = rxjs.noop, supportsExpand, } = {}) {
    ...
    function dfs(drillNodes, depth) {
        ...
        for (const node of drillNodes) {
            ...
            if (treeMode.isLeafNode(node) && !node.hasChild) {
                ...
                if (!supportsExpand || (enforceExpandTotalNode && node.key === totalKey)) {
                    ...
                    // 展开的父节点
                    crossTreeNode.title = (React__default.createElement(ExpandSpan, { onClick: () => {
                            onChangeExpandKeys(expandKeys.filter((k) => k !== node.key), node, 'collapse');
                        } },
                        React__default.createElement(treeMode.CaretRight16, { className: "icon expanded" }),
                        node.value));
                    crossTreeNode.children = dfs(node.children);
                }
                else {
                    ...
                    crossTreeNode.title = (React__default.createElement(ExpandSpan, { onClick: () => {
                        ...
                        React__default.createElement(treeMode.CaretRight16, { className: "icon collapsed" }),
                        node.value));
                    if (indicators != null) {
...
function buildCrossTable(options) {
    ...
    const leftHeaderWidth = Math.max(leftMetaColumns.length, treeMode.getTreeDepth(leftTree) + 1);
    return {
        columns: getColumns(),
    ...
    function getColumns() {
        ...
        function getLeftPartColumns() {
            ...
            for (let index = 0; index < leftHeaderWidth; index++) {
                ...
                const staticMetaColConfig = treeMode.__rest(metaCol, ["getCellProps", "render"]);
            ...
            function leftHeaderGetCellPropsFactory(metaCol, colIndex) {
                return (_value, row, rowIndex) => {
                    ...
                    return Object.assign({ className: cx(CrossTableClasses.leftHeaderCell, customProps === null || customProps === void 0 ? void 0 : customProps.className) }, customProps);
                };
            }
        ...
        function getDataPartColumns() {
            ...
            function dfs(nodes, ctx) {
                ...
                for (const node of nodes) {
                    ...
                    if (treeMode.isLeafNode(node)) {
                        // 叶子节点
                        result.push(getDataColumn(node, ctx.depth));
                    }
                    else {
                        const { key, value, children } = node, others = treeMode.__rest(node
                        // 强制展开的节点
                        , ["key", "value", "children"]);
                        // 强制展开的节点
        ...
        function getDataColumn(topNode, topDepth) {
            ...
            const { key, value, children } = topNode, others = treeMode.__rest(topNode, ["key", "value", "children"]);
...
const StyledBaseTableForCrossTable = styled__default(treeMode.BaseTable) `
  // 使 art-table 默认的 hover 效果失效
  .${treeMode.Classes.tableBody} tr.hovered {
    background-color: inherit;
  }

  .${treeMode.Classes.tableBody} td.${CrossTableClasses.leftHeaderCell} {
...
function CrossTable(_a) {
    var { leftTree, leftTotalNode, topTree, topTotalNode, getValue, getCellProps, leftMetaColumns, render, className } = _a, others = treeMode.__rest(_a, ["leftTree", "leftTotalNode", "topTree", "topTotalNode", "getValue", "getCellProps", "leftMetaColumns", "render", "className"]);
    ...
    return (React__default.createElement(StyledBaseTableForCrossTable, Object.assign({}, others, { dataSource: dataSource, columns: columns, primaryKey: ROW_KEY, className: cx('compact', className) })));
}

function buildCrossTreeTable(options) {
    const { leftTree, topTree, primaryColumn, openKeys, onChangeOpenKeys, indentSize, isLeafNode: isLeafNodeOpt = treeMode.isLeafNode, } = options;
            ...
            function dfs(nodes, ctx) {
                ...
                for (const node of nodes) {
                    if (treeMode.isLeafNode(node)) {
                        ...
                        const { key, value, children } = node, others = treeMode.__rest(node, ["key", "value", "children"]);
...
class CrossTreeTable extends React__default.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        return React__default.createElement(treeMode.BaseTable, Object.assign({}, others, { primaryKey: ROW_KEY, dataSource: dataSource, columns: columns }));
    }
}
...
exports.CrossTableClasses = CrossTableClasses;
exports.CrossTreeTable = CrossTreeTable;
exports.ROW_KEY = ROW_KEY;
exports.StyledBaseTableForCrossTable = StyledBaseTableForCrossTable;
